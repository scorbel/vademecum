...
public class MatrixVectorMult {
		...
		protected void reduce(IntWritable key, Iterable<Text> values, Context context)
			...
			for (Text value : values) {
				...
				if (splits.length == 1) {
					...
					matrixColumn.put(Integer.valueOf(splits[0]), Double.valueOf(splits[1]));
				}
			}
	...
	public static void job(Configuration conf) throws IOException, ClassNotFoundException, InterruptedException {
		...
		FileInputFormat.setInputPaths(job1,
				new Path[] { new Path(conf.get("initialVectorPath")), new Path(conf.get("inputMatrixPath")) });
		...
		// If your implementation of the combiner passed the unit test,
